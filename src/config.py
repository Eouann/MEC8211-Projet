"""
Fichier de configuration des constantes
"""


# Définition des constantes
alpha=0.3e-6        # m²/s (matière réfractaire)
cp=1000             # J/(kg.K) (capacité thermique massique de la matière réfractaire)
rho=3000            # kg/m³ (masse volumique de la matière réfractaire)
e=0.1               # m (épaisseur du mur du four)
T_0=20+273          # K (température initiale du mur)
T_x_0=1600+273      # K (température de la flamme du four)
T_x_inf=20+273      # K (température de l'air ambiant dans l'atelier)
t_max=3600          # s (durée de la simulation)
h=10                # W/(m².K) (coefficient de convection)
